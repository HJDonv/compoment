...
void hsm_dispatch(sm_t *me, sm_event_t *e)
{
	...
	do
	{
		...
		{
			// 可能由于guard而unhandle，将消息丢给super去处理，
			// 通过EMPTY信号触发ret = SM_RET_SUPER.
	...
	if(SM_RET_TRAN == ret)
	{
		...
		path[0] = me->temp; 	// 状态转换的目的状态transition target state
		path[1] = t; 			// 状态转换的源状态transition source state
		...
		for( ; s != t; t = me->temp)
		{
			...
			if(SM_RET_HANDLED == ret)
			{
				SM_TRIG(me, t, SM_EMPTY_SIG); /* find superstate of t */
			}
		}
